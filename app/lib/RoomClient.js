...
export default class RoomClient {
	...
	constructor(
		...
		this._externalLinkProducer = null;
	...
	async join() {
		const protooTransport = new protooClient.WebSocketTransport(this._protooUrl, {
			protocols: ['protoo'],
			requestOptions: {
				requestedProtocols: ['protoo']
			}
		});

		this._protoo = new protooClient.Peer(protooTransport);

		console.log('protooTransport', protooTransport, this._protoo)
	...
	async enableExternalLink() {
		logger.debug('enableExternalLink()');

		if (this._externalLinkProducer)
			return;
		// else if (this._shareProducer)
		// 	await this.disableShare();

		if (!this._mediasoupDevice.canProduce('video')) {
			logger.error('enableExternalLink() | cannot produce video');
		...
		try {
			...
			this._externalLinkProducer = await this._sendTransport.produce(
				{
					track,
					encodings,
					codecOptions,
			...
			if (this._e2eKey && e2e.isSupported()) {
				e2e.setupSenderTransform(this._externalLinkProducer.rtpSender);
			}

			store.dispatch(stateActions.addProducer(
				{
					id: this._externalLinkProducer.id,
					deviceLabel: device.label,
					type: this._getWebcamType(device),
					paused: this._externalLinkProducer.paused,
					track: this._externalLinkProducer.track,
					rtpParameters: this._externalLinkProducer.rtpParameters,
					codec: this._externalLinkProducer.rtpParameters.codecs[0].mimeType.split('/')[1]
				}));

			this._externalLinkProducer.on('transportclose', () => {
				this._externalLinkProducer = null;
			});

			this._externalLinkProducer.on('trackended', () => {
				store.dispatch(requestActions.notify(
					{
						type: 'error',
						text: 'Webcam disconnected!'
			...
			logger.error('enableExternalLink() | failed:%o', error);
	...
	async pauseWebcam() {
		...
		try {
			...
			logger.error('pauseWebcam() | failed: %o', error);
	...
	async pauseProducer(producer) {
		...
		try {
			await this._protoo.request(
				'pauseProducer', { producerId: producer.id });

			store.dispatch(stateActions.setProducerPaused(producer.id))

		}
		catch (error) {
			logger.error('pauseProducer() | failed: %o', error);
	...
	async resumeProducer(producer) {
		...
		try {
			await this._protoo.request(
				'resumeProducer', { producerId: producer.id });

			store.dispatch(stateActions.setProducerResumed(producer.id))

		}
		catch (error) {
			logger.error('resumeProducer() | failed: %o', error);
	...
	async _joinRoom() {
		...
		try {
			...
			if (isHost) {
				store.dispatch(stateActions.setAsHost());
			}
